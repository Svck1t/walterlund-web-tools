...
const ReposicionScan = (() => {
  ...
  async function getTextWorker(forceNew = false) {
    if (forceNew && textWorker) {
      try { await textWorker.terminate(); } catch (_) { /* ya estaba roto, se descarta igual */ }
      textWorker = null;
    }
  ...
  async function getDigitWorker(forceNew = false) {
    if (forceNew && digitWorker) {
      try { await digitWorker.terminate(); } catch (_) { /* ya estaba roto, se descarta igual */ }
      digitWorker = null;
    }
    if (!digitWorker) {
  ...
  /**
   * Corre `recognize` y, si el motor de OCR falla en la primera pasada
   * (típico problema de carga del núcleo WASM de Tesseract.js), descarta
   * ese worker, crea uno nuevo y reintenta una vez antes de rendirse.
   */
  async function recognizeWithRetry(getWorker, image) {
    try {
      const worker = await getWorker(false);
      return await worker.recognize(image);
    } catch (err) {
      const worker = await getWorker(true); // worker nuevo
      return await worker.recognize(image); // si vuelve a fallar, se propaga el error real
    }
  }
  ...
  /** Falla temprano y con mensaje claro si algún script del CDN no cargó. */
  function checkLibraries() {
    const missing = [];
    if (typeof window.pdfjsLib === 'undefined') missing.push('pdf.js');
    if (typeof window.Tesseract === 'undefined') missing.push('Tesseract.js');
    if (missing.length > 0) {
      throw new Error(
        `No se pudo cargar: ${missing.join(', ')}. Revisa tu conexión a internet ` +
        `(estas librerías se cargan desde un CDN externo) y recarga la página.`
      );
    }
  }
  ...
  async function scan(file, referenceRecords, onProgress) {
    ...
    let pages;
    try {
      pages = await PdfScanReader.renderPages(file);
    } catch (err) {
      throw new Error(`No se pudo renderizar el PDF (pdf.js): ${describeError(err)}`);
    }
    ...
    const results = [];
    // Verificamos que el worker de texto pueda crearse al menos una vez
    // (si esto falla, no tiene sentido seguir).
    try {
      await getTextWorker();
    } catch (err) {
      throw new Error(`No se pudo iniciar el motor de OCR (Tesseract): ${describeError(err)}`);
    }
    ...
    for (let p = 0; p < pages.length; p++) {
      ...
      let data;
      try {
        ({ data } = await recognizeWithRetry(getTextWorker, canvas));
      } catch (err) {
        throw new Error(`Falló el OCR de texto en la página ${p + 1}: ${describeError(err)}`);
      }
      ...
      for (const w of words) {
        ...
        let cropData;
        try {
          ({ data: cropData } = await recognizeWithRetry(getDigitWorker, cropCanvas));
        } catch (err) {
          throw new Error(`Falló el OCR de la cantidad para "${ref.codigo}": ${describeError(err)}`);
        }
